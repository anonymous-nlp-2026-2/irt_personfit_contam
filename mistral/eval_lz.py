"""
eval_lz.py — Evaluate a fine-tuned Mistral adapter on full MMLU and compute lz* person-fit statistic.
...
def main():
    ...
    print(f"Loaded IRT params for {len(irt_params)} items")

    mmlu_path = os.path.join(BASE_DIR, "data", "mmlu_items.json")
    with open(mmlu_path) as f:
        mmlu_items = json.load(f)
    print(f"Loaded {len(mmlu_items)} MMLU items")
    ...
    with open(seen_15_path) as f:
        seen_15 = set(json.load(f))
    with open(seen_50_path) as f:
        seen_50 = set(json.load(f))
    ...
    if "contam_50" in args.condition:
        ...
        seen_set = set()
    print(f"Condition: {args.condition}, seen items: {len(seen_set)}")
